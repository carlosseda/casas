...
module.exports = async function scrappingLocations (urlBase) {
  ...
  const sublocationsUrls = []
  const sublocationsElements = await scrappingService.driver.findElements(scrappingService.by.css('.modal-wrapper #sublocations li a'))
...
async function urlsIterator (scrappingService, url) {
  ...
  try {
    ...
    if (btnShowAll) {
      ...
      const locationsElements = await scrappingService.driver.findElements(scrappingService.by.css('.modal-wrapper #sublocations li a'))
      ...
      const locationsElements = await scrappingService.driver.findElements(scrappingService.by.css('#sublocations li a'))
